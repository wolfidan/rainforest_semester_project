...
def vert_aggregation(
    radar_data, vert_weights, grp_vertical, visib_weight=True, visib=None
):
    """
    ...
    if visib_weight and not np.any(visib is None):
        vert_weights = vert_weights * visib / 100.0
    else:
    ...
    for v in radar_data.columns:
        if v not in ["RADAR", "HYDRO", "TCOUNT"]:
            X[v] = (radar_data[v] * vert_weights).groupby(
                grp_vertical
            ).sum() / sum_wvisib
        else:
            ...
            for val in vals:
                X[v + "_prop_" + str(val)] = (
                    (radar_data[v] == val).astype(int) * vert_weights
...
def _polyfit_no_inter(x, y, degree):
    ...
    p = np.insert(
        p, 0, 0
    )  # Add zero intercept at beginning for compatibility with polyval
...
class RandomForestRegressorBC(RandomForestRegressor):
    ...
    def fit(
        ...
        try:
            ...
            raise
        vweights = 10 ** (self.beta * (heights / 1000.0))  # vert. weights
        ...
        features_VERT_AGG = vert_aggregation(
            X, vweights, grp_vertical, visib_weighting, visib
        )
