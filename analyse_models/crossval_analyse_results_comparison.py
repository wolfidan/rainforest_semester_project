...
# DANIELS METRICS-PLOTTING FILE MODIFIED TO COMPARE TWO ARBITRARY MODELS
# data has to be in the same format as "/scratch/mch/wolfensb/rainforest_semester_project/saved_models/GRU_Baseline"
...
ALLFOLD_FILES = { # only comparision between TWO models supported
    MODEL_1_NAME: MODEL_1_PATH,
...
def compute_metrics_table_allfolds(bounds):
    ...
    """
    rows = []
    for method in [MODEL_1_NAME, MODEL_2_NAME]:
...
def save_boxplot_compare(df, outdir, split, metric, bounds_order):
    ...
    for ax, b in zip(axes, bounds_order):
        db = d[d["bound"] == b]
        vals_gru = db.loc[db["method"] == MODEL_1_NAME, "value"].dropna().to_numpy()
        vals_rf  = db.loc[db["method"] == MODEL_2_NAME,  "value"].dropna().to_numpy()
        print(b, vals_gru, vals_rf, metric)
        ax.boxplot([vals_gru, vals_rf], labels=[MODEL_1_NAME, MODEL_2_NAME], showmeans=True)
...
def save_summary_bar(df, outdir, split, metric, bounds_order):
    ...
    m_gru, s_gru, c_gru = extract(MODEL_1_NAME)
