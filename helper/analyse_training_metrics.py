...
def analyze_training_results(stats_path):
    ...
    final_stats = df[df['epoch'] == last_epoch].groupby('split')['mse'].mean()
    ...
    best_val_per_fold = df[df['split'] == 'val'].groupby('fold')['mse'].min()
    ...
    mean_val_by_epoch = df[df['split'] == 'val'].groupby('epoch')['mse'].mean()
